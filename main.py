...
MY_NAME = const("Christian")                  # Your own handle
...
while True:
    ...
    if msg:
        ...
        if msg[0] == '*':
            if SHOW_BROADCAST_MESSAGES == True: # Only show broadcast messages if wanted. Control in Configuration
                lcd.print_broadcast(msg)
                print("Broadcast " + mac_addr + ": " + msg[1:]) # Remove the broadcast identifyer *
        else:
    ...
    if BROADCAST_INTERVAL > 0:
        if time.ticks_diff(time.ticks_ms(), time_last_broadcast) > BROADCAST_INTERVAL:
            ...
            en.esp_now_send_message(en.MAC_ADDR_BROADCAST, "*" + MY_NAME + ": " + str(time_last_broadcast))
